  ...
  function setupEventHandlers() {
    ...
    $('#btnAddBlogTag').click(function(e) {
      e.preventDefault();
      showBlogTagModal();
    });
    ...
    $(document).on('click', '.btn-delete-blog-comment', function() {
      const id = parseInt($(this).data('id'), 10);
      deleteBlogComment(id);
    });

    $('#blogCommentFilter').on('change', function() {
      loadBlogComments();
    });
  }
  ...
  function showSection(section) {
    ...
    const titles = {
      ...
      blogs: 'Quản lý Blog',
      'blog-tags': 'Quản lý Thẻ Blog',
      'blog-comments': 'Quản lý Bình luận Blog'
    };
    ...
    setTimeout(() => {
      if (section === 'dashboard') {
        ...
        loadBlogs();
      } else if (section === 'blog-tags') {
        loadBlogTags();
      } else if (section === 'blog-comments') {
        loadBlogComments();
      }
    }, 100);
  }
  ...
  // Blogs Management
  function loadBlogs(updateStats = false) {
    return $.ajax({
      ...
      success: function(response) {
        ...
        if (updateStats) {
          $('#stat-blogs').text(blogsArray.length);
        } else {
          renderBlogsTable(blogsArray);
        }
      },
      error: function(xhr) {
        console.error('Error loading blogs:', xhr);
        if (!updateStats) {
          $('#blogsTableBody').html('<tr><td colspan="8" class="text-center text-danger">Lỗi tải dữ liệu: ' + (xhr.responseJSON?.message || 'Không thể tải dữ liệu') + '</td></tr>');
        }
      }
    });
  }
  ...
  function renderBlogsTable(blogs) {
    ...
    blogs.forEach(blog => {
      const userDisplay = blog.user 
        ? `${blog.user.email}${blog.user.name ? ' (' + blog.user.name + ')' : ''}`
        : (blog.userId || '-');
      ...
      const row = `
        <tr>
          ...
          <td>
            <button class="btn btn-sm btn-info btn-edit-blog" data-id="${blog.id}" title="Chỉnh sửa"><i class="fas fa-edit"></i></button>
            <button class="btn btn-sm btn-danger btn-delete-blog" data-id="${blog.id}" title="Xóa"><i class="fas fa-trash"></i></button>
          </td>
        </tr>
      `;
      tbody.append(row);
  ...
  function showBlogModal(blogId = null) {
      ...
      <div class="modal fade" id="blogModal" tabindex="-1" role="dialog">
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title">${blogId ? 'Chỉnh sửa' : 'Thêm mới'} Blog</h5>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
              ...
              <form id="blogForm">
                ...
                <div class="row">
                  <div class="col-md-6">
                    <div class="form-group">
                      <label>Loại *</label>
                      <select class="form-control" id="blogType" required>
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                      ...
                      <select class="form-control" id="blogCategory" required>
                        ...
                        <option value="MANAGEMENT_STAFF">Nhân viên quản lý</option>
                        <option value="NEW_TECHNOLOGY">Công nghệ mới</option>
                      </select>
                    </div>
                  </div>
                </div>
                <div class="form-group">
                  <label>Hình ảnh</label>
                  <div class="mb-2">
                    <input type="file" class="form-control-file" id="blogImgFile" accept="image/*" style="display: none;">
                    <button type="button" class="btn btn-sm btn-primary" id="btnUploadBlogImage">
                      <i class="fa fa-upload"></i> Upload ảnh
                    </button>
                    <span class="ml-2 text-muted">hoặc</span>
                    <button type="button" class="btn btn-sm btn-secondary ml-2" id="btnUseBlogUrl">
                      Nhập URL
                    </button>
                  </div>
                  <div id="blogImagePreviewContainer" class="mb-2" style="display: none;">
                    <img id="blogImagePreview" src="" alt="Preview" style="max-width: 200px; max-height: 200px; border: 1px solid #ddd; border-radius: 4px; padding: 5px;">
                    <button type="button" class="btn btn-sm btn-danger ml-2" id="btnRemoveBlogImage">
                      <i class="fa fa-times"></i> Xóa
                    </button>
                  </div>
                  <input type="text" class="form-control" id="blogImg" placeholder="URL ảnh hoặc upload ảnh phía trên">
                </div>
              </form>
            </div>
            <div class="modal-footer">
              <button type="button" class="btn btn-secondary" data-dismiss="modal">Đóng</button>
              <button type="button" class="btn btn-primary" id="btnSaveBlog">Lưu</button>
            </div>
          </div>
        </div>
      </div>
    ...
    // Bind save button event
    $('#btnSaveBlog').off('click').on('click', function() {
      saveBlog();
    });
    ...
    if (blogId) {
      $.ajax({
        ...
        method: 'GET',
        success: function(response) {
          const blog = extractData(response);
          $('#blogTitle').val(blog.title);
          $('#blogContent').val(blog.content);
          $('#blogType').val(blog.type);
          $('#blogCategory').val(blog.category);
          
          const imgUrl = blog.image || '';
          $('#blogImg').val(imgUrl);
          if (imgUrl) {
            $('#blogImagePreview').attr('src', imgUrl);
            $('#blogImagePreviewContainer').show();
          }
        },
        error: function(xhr) {
          alert('Lỗi tải thông tin blog: ' + formatErrorMessage(xhr));
        }
      });
    }
  }
  ...
  function saveBlog() {
    ...
    const data = {
      ...
      image: $('#blogImg').val() || null
    };
    
    const url = blogId ? `${API_BASE_URL}/blogs/${blogId}` : `${API_BASE_URL}/blogs`;
    const method = blogId ? 'PATCH' : 'POST'; // Assuming default behavior, usually PUT or PATCH. Admin.js used PUT for others, but let's stick to consistent style. Wait, user creation used PATCH, course used PUT. I will check schema... Schema doesn't say method. I'll use PATCH for partial updates usually.
    // However, existing code uses PUT for course, PATCH for user. I'll try PATCH first as it's safer.
  ...
  function setupBlogImageUpload() {
    // Similar to course image upload
    $('#btnUploadBlogImage').off('click').on('click', function() {
      $('#blogImgFile').click();
    });
    
    $('#blogImgFile').off('change').on('change', function(e) {
      const file = e.target.files[0];
      if (!file) return;
      if (!file.type.startsWith('image/')) {
      ...
      $.ajax({
        url: `${API_BASE_URL}/blogs/upload-image`, // Assuming this endpoint exists, or generic upload
        method: 'POST',
        data: formData,
        processData: false,
        contentType: false,
        success: function(response) {
          const imageUrl = response.data?.url || response.url;
          if (imageUrl) {
            $('#blogImg').val(imageUrl);
            $('#blogImagePreview').attr('src', imageUrl);
            $('#blogImagePreviewContainer').show();
          }
        },
        error: function(xhr) {
          // Fallback if specific route doesn't exist, try Generic or Course upload
           $.ajax({
            url: `${API_BASE_URL}/courses/upload-image`,
            method: 'POST',
            data: formData,
            ...
            error: function(xhr2) {
               alert('Lỗi upload ảnh: ' + formatErrorMessage(xhr2));
               $('#blogImagePreviewContainer').hide();
               $('#blogImgFile').val('');
            }
           });
        }
    ...
    $('#btnUseBlogUrl').off('click').on('click', function() {
      $('#blogImg').focus();
      $('#blogImagePreviewContainer').hide();
      $('#blogImgFile').val('');
    });
    
    $('#btnRemoveBlogImage').off('click').on('click', function() {
      $('#blogImg').val('');
  ...
  function loadBlogTags(updateStats = false) {
    return $.ajax({
      url: `${API_BASE_URL}/blog-tags`,
      method: 'GET',
      success: function(response) {
        const tags = extractData(response);
        const tagsArray = Array.isArray(tags) ? tags : [];
        
        if (updateStats) {
          $('#stat-blog-tags').text(tagsArray.length);
        } else {
          renderBlogTagsTable(tagsArray);
        }
      },
      error: function(xhr) {
        console.error('Error loading blog tags:', xhr);
        if (!updateStats) {
          $('#blogTagsTableBody').html('<tr><td colspan="4" class="text-center text-danger">Lỗi tải dữ liệu: ' + formatErrorMessage(xhr) + '</td></tr>');
        }
      }
    });
  }

  function renderBlogTagsTable(tags) {
    const tbody = $('#blogTagsTableBody');
    tbody.empty();
    ...
    tags.forEach(tag => {
      const row = `
        <tr>
          ...
          <td>
            <button class="btn btn-sm btn-info btn-edit-blog-tag" data-id="${tag.id}" title="Chỉnh sửa"><i class="fas fa-edit"></i></button>
            <button class="btn btn-sm btn-danger btn-delete-blog-tag" data-id="${tag.id}" title="Xóa"><i class="fas fa-trash"></i></button>
          </td>
        </tr>
      `;
      tbody.append(row);
  ...
  function showBlogTagModal(tagId = null) {
    const modalHtml = `
      <div class="modal fade" id="blogTagModal" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title">${tagId ? 'Chỉnh sửa' : 'Thêm mới'} Thẻ Blog</h5>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
            <div class="modal-body">
              <form id="blogTagForm">
                <input type="hidden" id="blogTagId" value="${tagId || ''}">
                <div class="form-group">
                  <label>Tên thẻ *</label>
                  <input type="text" class="form-control" id="blogTagName" required>
                </div>
              </form>
            </div>
            <div class="modal-footer">
              <button type="button" class="btn btn-secondary" data-dismiss="modal">Đóng</button>
              <button type="button" class="btn btn-primary" id="btnSaveBlogTag">Lưu</button>
            </div>
          </div>
        </div>
      </div>
    `;
    $('#modalContainer').html(modalHtml);
    $('#blogTagModal').modal('show');
    
    $('#btnSaveBlogTag').off('click').on('click', function() {
      saveBlogTag();
    });
    
    if (tagId) {
      $.ajax({
        url: `${API_BASE_URL}/blog-tags/${tagId}`,
        method: 'GET',
        success: function(response) {
          const tag = extractData(response);
          $('#blogTagName').val(tag.name);
        },
        error: function(xhr) {
          alert('Lỗi tải thông tin thẻ: ' + formatErrorMessage(xhr));
        }
      });
    }
  }
  ...
  function saveBlogTag() {
    const tagId = $('#blogTagId').val();
    const data = {
      name: $('#blogTagName').val()
    };
    ...
    const url = tagId ? `${API_BASE_URL}/blog-tags/${tagId}` : `${API_BASE_URL}/blog-tags`;
  ...
  function loadBlogComments(updateStats = false) {
    // If just stat, try to get all or count. 
    // Assuming /blog-comments endpoint exists for admin or we use filter
    
    let url = `${API_BASE_URL}/blog-comments`;
    const blogId = $('#blogCommentFilter').val();
    
    if (blogId) {
       // Assuming endpoint for filtering
       // API might vary. checking standard pattern.
       // The existing comments used `/video/lessons/...` but this is blog.
       // I'll guess `/blog-comments` takes query param or `/blogs/{id}/comments`.
       // Let's assume `/blog-comments` returns all list if we are admin.
       // Or `?blogId=` query
    }
    ...
    return $.ajax({
      url: url,
      method: 'GET',
      data: blogId ? { blogId: blogId } : {},
      success: function(response) {
        const comments = extractData(response);
        const commentsArray = Array.isArray(comments) ? comments : [];
        
        if (updateStats) {
          $('#stat-blog-comments').text(commentsArray.length);
        } else {
          renderBlogCommentsTable(commentsArray);
          
          // Load blogs for filter if empty
          if ($('#blogCommentFilter option').length <= 1) {
             $.ajax({
               url: `${API_BASE_URL}/blogs`,
               method: 'GET',
               success: function(r) {
                 const blogs = extractData(r);
                 const sel = $('#blogCommentFilter');
                 (Array.isArray(blogs) ? blogs : []).forEach(b => {
                   sel.append(`<option value="${b.id}">${b.title}</option>`);
                 });
               }
             });
          }
      ...
      error: function(xhr) {
        console.error('Error loading blog comments:', xhr);
        if (!updateStats) {
          $('#blogCommentsTableBody').html('<tr><td colspan="6" class="text-center text-danger">Lỗi tải dữ liệu: ' + formatErrorMessage(xhr) + '</td></tr>');
        }
      }
    });
  }

  function renderBlogCommentsTable(comments) {
    const tbody = $('#blogCommentsTableBody');
    tbody.empty();
    ...
    comments.forEach(comment => {
      const content = comment.content || '';
      const preview = content.length > 50 ? content.substring(0, 50) + '...' : content;
      const userDisplay = comment.user 
        ? `${comment.user.email}${comment.user.name ? ' (' + comment.user.name + ')' : ''}`
        : (comment.userId || '-');
      ...
      const row = `
        <tr>
          ...
          <td>${preview}</td>
          <td>${userDisplay}</td>
          <td>${comment.blog?.title || comment.blogId || '-'}</td>
          <td>${formatDate(comment.createdAt)}</td>
          <td>
            <button class="btn btn-sm btn-danger btn-delete-blog-comment" data-id="${comment.id}" title="Xóa"><i class="fas fa-trash"></i></button>
          </td>
        </tr>
      `;
      tbody.append(row);
  ...
  function deleteBlogComment(id) {
    if (confirm('Bạn có chắc chắn muốn xóa bình luận này?')) {
      $.ajax({
        url: `${API_BASE_URL}/blog-comments/${id}`,
        method: 'DELETE',
        success: function() {
          loadBlogComments();
          alert('Xóa thành công!');
  ...
  window.saveBlogTag = saveBlogTag;
